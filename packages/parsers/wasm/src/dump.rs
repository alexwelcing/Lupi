...
pub fn parse_dump(content: &str) -> Result<Vec<JsValue>, JsError> {
    let frames = parse_dump_frames(content)
        .map_err(|e| JsError::new(&format!("Parse error: {}", e)))?;

    frames
        .into_iter()
        .map(|f| serde_wasm_bindgen::to_value(&f).map_err(|e| JsError::new(&e.to_string())))
        .collect()
}

/// Parse a single frame from a dump file by index (0-based).
/// More memory-efficient than parsing all frames for large files.
#[wasm_bindgen(js_name = "parseDumpFrame")]
pub fn parse_dump_frame(content: &str, frame_index: usize) -> Result<Frame, JsError> {
    let frames = parse_dump_frames(content)
        .map_err(|e| JsError::new(&format!("Parse error: {}", e)))?;
    ...
    frames
        .into_iter()
        .nth(frame_index)
        .ok_or_else(|| JsError::new(&format!("Frame {} not found", frame_index)))
}
...
#[wasm_bindgen(js_name = "countDumpFrames")]
...
#[wasm_bindgen(js_name = "indexDumpFrames")]
...
/// Parse all frames from a dump string.
pub(crate) fn parse_dump_frames(content: &str) -> Result<Vec<Frame>, String> {
    ...
    while lines.peek().is_some() {
        ...
        match parse_single_frame(&mut lines) {
            ...
            Err(e) => {
                // Log warning but continue parsing remaining frames
                web_sys::console::warn_1(&format!("Skipping frame: {}", e).into());
                continue;
            }
        }
    }
    ...
    I: Iterator<Item = &'a str>,
{
    ...
    for dim in 0..3 {
        ...
        if triclinic && parts.len() >= 3 {
            // Triclinic: xlo_bound xhi_bound xy (or xz, yz)
