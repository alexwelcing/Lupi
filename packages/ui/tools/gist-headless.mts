...
//   pnpm scan:gist:headless -- apple      (or screw, mug)
//
...
const engine = createGistEngine({ gpu, target: colorTarget, shaders: { step: step.wgsl, points: points.wgsl }, aspect: () => width / height, random });
...
async function glowStats() {
  ...
  let glow = 0;
  let radius = 0;
  for (let i = 0; i < GIST_PARTICLE_COUNT; i += 1) {
    const base = i * PARTICLE_FLOATS;
    glow += raw[base + 7];
    radius += Math.hypot(raw[base], raw[base + 1], raw[base + 2]);
  }
  return { meanGlow: glow / GIST_PARTICLE_COUNT, meanRadius: radius / GIST_PARTICLE_COUNT };
}
