...
describe('science route ⇄ gallery mapping', () => {
  it('maps and round-trips every canonical path', () => {
    for (const index of [0, 14, 16, 27]) {
      const id = scienceGalleryIdForPathIndex(index);
      expect(id).toBe(`z1_science_path_${index}`);
      expect(sciencePathIndexForGalleryId(id!)).toBe(index);
    }
