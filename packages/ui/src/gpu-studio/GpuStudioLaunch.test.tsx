...
vi.mock("../store", () => ({
  useStore: Object.assign(
    (selector: (state: any) => unknown) => selector(mocks.state),
    {
...
describe("GPU Studio launch lifecycle", () => {
  ...
  it("turns initialization rejection into a closable failure, not a broken viewer", async () => {
    ...
    expect(
      screen.getByRole("button", { name: "Rotate", exact: true }),
    ).toHaveProperty("disabled", true);
    fireEvent.click(
      screen.getByRole("button", { name: /Return to my molecule/ }),
    );
    expect(screen.queryByRole("dialog")).toBeNull();
  });
  it("wires the finishes, light and atom focus without modifying the source frame", async () => {
    ...
    expect(
      screen
        .getByRole("button", { name: /Snowglobe/ })
        .getAttribute("aria-pressed"),
    ).toBe("true");
    fireEvent.click(screen.getByRole("button", { name: "Shake it" }));
    expect(instance.shake).toHaveBeenCalledTimes(1);
    fireEvent.click(screen.getByRole("button", { name: "Settle snow" }));
    expect(instance.calm).toHaveBeenCalledTimes(1);
    expect(
      screen
        .getByRole("button", { name: "Enable phone motion" })
        .getAttribute("aria-pressed"),
    ).toBe("false");
    expect(
      screen
        .getByRole("button", { name: "All atoms" })
        .getAttribute("aria-pressed"),
    ).toBe("true");
