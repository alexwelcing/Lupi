...
Compatibility snapshot: **2026-08-17**
...
This document describes the verified runtime source on exact main revision
`0e78b6b4a4493cae31019bbcb9aa5b01c6ae32a0`, plus clearly identified receipts
from earlier signed revisions. For the Apple Silicon development-machine
transition, also read the
...
checkpoint has a signed SDK 57 internal artifact at SHA `7cd75aaf` and an
installation/partial-Viewer receipt on iPhone 15 Pro. It is not an Expo Go,
post-fix, TestFlight, or Room AR acceptance receipt.
...
- Do not change Expo, React Native, React, Router, or Expo packages piecemeal.
  ...
  acceptance waits for installing and exercising a signed current-candidate build.
...
The SDK 57 source has reached a complete, landed local/configuration checkpoint
and a partial physical-device milestone. Complete SDK 55 and SDK 56 local
receipts remain at commits `1a56e398` and `42536acd`. A matching same-runtime
development update is active; the post-fix physical-device retest remains open:
...
| Integrated candidate        | Exact clean main `0e78b6b4…`; exact-head source and build-test CI passed; EAS profiles pin Node `22.23.1` and the repository pins pnpm `9.0.0`                                                                                                         | Post-fix physical-device behavior and production/TestFlight gates         |
...
| Development update          | Active iOS update `01a0101e…`, group `11ec17f2-4047-4e80-b688-a769fb892668`, channel `development`, runtime `1.0.1`, exact clean `0e78b6b4`, Lupi Dev identity                                                                                         | Post-fix device install/gesture/Room receipt                              |
...
- `/viewer` is a root Stack detail route, separate from the persistent tab bar.
  It delegates to `ViewerScreen`, provides an explicit native Back button, and
  disables interactive back-swipe so horizontal molecular rotation cannot pop
  the route. It loads only the
...
- A typed bridge injects serialized `lupi.*` request objects into the page's
  ...
  the iOS share sheet.
- `EXPO_PUBLIC_LUPI_WEB_URL` defaults to `https://lupi.live` and is used for the
  WebView. Native Gallery filtering remains on device; mobile does not currently
  issue `/mcp` search calls.
...
`2026-08-14T11:50:14.750703Z`. The edge manifest contains exactly seven tools
...
simulator builds. The required `mobile-visual.yml` and reusable
`mobile-visual-capture.yml` workflows therefore stop at the Caffeine viewer and
native Gallery/Library/Settings shells. AR is isolated in the manual,
capture-only `mobile-visual-ar-diagnostic.yml` workflow, which accepts the same
successful simulator `build_id`. Its failure cannot block the required shell
receipts and cannot serve as a Room receipt. Even if that diagnostic succeeds,
it cannot prove camera permission, ARKit tracking, plane discovery, placement,
occlusion, or real-room interaction.
...
| Native Room AR                         | Viro 2.57.5 / ARKit source behind strict `export_xyz` handoff; 512 atoms, 2,048 inferred bonds, opaque in-memory session; signed install exists but Room has no device receipt                                                   | Expo Go shows the explicit development-build boundary only                                                                                  | Physical iPhone placement, gesture, tracking, privacy, performance, and lifecycle acceptance |
...
| Viewer camera/style controls           | Compact Fit/Camera/Look/More/Share/Room toolbar with iOS action sheets and accessible fallback; route back-swipe disabled so horizontal drags rotate instead of dismissing; other actions cross typed boundaries                 | Post-fix physical rotation/back-button retest plus action-sheet, playback, Room fallback, and error receipts                                | Native controls drive native scene state                                                     |
...
| ------------------------- | ------------------------------------------------------ | --------------------------------------------------------------------------------------------------------------- |
| Runtime/tooling           | Node 22.23.1 and pnpm 9.0.0                            | Exact frozen-install and release-verification toolchain used for the landed main checkpoint                     |
...
| Active development update | Runtime `1.0.1`, update `01a0101e…`, group `11ec17f2…` | Exact clean `0e78b6b4`, Lupi Dev identity; post-fix device retest remains                                       |
...
development-channel update is active for runtime `1.0.1`: update
`01a0101e-640a-7d5e-b248-6ae762f7a50f`, group
`11ec17f2-4047-4e80-b688-a769fb892668`, exact clean revision `0e78b6b4`. It
resolves as the Lupi Dev identity and is compatible with the installed `1.0.1`
binary. The post-fix device retest remains open. Before every later
...
`7cd75aaf346f362f29bb51d6b22677fb44e1e644` is live at the configured origin;
public `/health` identifies that tag and timestamp
`2026-08-14T11:50:14.750703Z`; the browser manifest has exactly 30 tools,
