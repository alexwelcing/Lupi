...
Status: **Expo hybrid shell plus native Viro/ARKit Room source locally verified; the compatible 30-tool web bridge is live and an earlier Room revision has a signed development build, while the new 1.0.1/iOS 17.6 candidate still needs its own signed build, workflow run, TestFlight processing, and physical-iPhone AR acceptance**

Compatibility snapshot: **2026-08-10**
...
have passed their source/browser ladder. The fresh 92-file EAS archive matches
current source; the final integrated commit still needs its full SHA recorded.
...
`1.0.1`/iOS `17.6` source, an executed visual workflow, Apple upload, TestFlight
processing, App Store release, or physical-iPhone AR pass.

## Compatibility decision: Expo SDK 54 shell plus development-client AR

`apps/mobile/package.json` pins Expo `~54.0.35`, React Native `0.81.5`, and
React `19.1.0`. That is deliberate. During the current Expo transition, the
App Store build of Expo Go for a physical iPhone supports SDK 54. Apple does not
allow installing an older Expo Go build side-by-side on an iPhone. Expo's
current project guide therefore tells physical-device Expo Go users to use SDK
54, and its development-build FAQ records the one-SDK iPhone constraint.

Consequences:

- Do not upgrade the mobile app's Expo SDK piecemeal during the Expo Go phase.
- Keep Expo, React Native, React, Router, and Expo package versions aligned with
  SDK 54. Use `npx expo install`, not arbitrary package upgrades, for Expo
  dependencies.
- Use the current App Store Expo Go app on the iPhone. If the store client moves
  off SDK 54, either upgrade this app as one tested change or move development
  to the custom development client described below.
- Expo Go remains valid for the hybrid shell, but it cannot load the custom
  native `@reactvision/react-viro` runtime. Room work must use a development or
  TestFlight build on a physical ARKit iPhone.
- SDK 54 requires Node `20.19.x` or newer, targets React Native `0.81`, and has
  an Expo baseline iOS minimum of 15.1. Lupi now explicitly sets iOS `17.6`
  through `expo-build-properties` because embedded ViroKit requires that higher
  target. The repository already requires Node 20 or newer.
...
Node `20.19.x` release for the full monorepo install and verification lane.
...
isolated candidate instead passed a clean frozen filtered install under Node
`20.19.4` and pnpm `9.0.0` with lifecycle scripts enabled.
...
- [Expo SDK 54 compatibility table](https://docs.expo.dev/versions/v54.0.0/)
...
The integrated candidate has reached a complete local/configuration milestone.
Cloud receipts exist for an earlier clean Room revision, but no signed release
artifact exists for the new `1.0.1`/iOS `17.6` source:
...
| Integrated candidate        | Branch `codex/mobile-testflight-integration` based on deployed `ee0d8885`; Node `20.19.4`, pnpm `9.0.0`, frozen filtered install with lifecycle scripts                                             | Final clean integrated SHA                                                      |
| App source identity         | Version/runtime `1.0.1`, bundle `live.lupi.app`, explicit iOS `17.6`, EAS remote version source/build number `1`, production store/autoIncrement/Node `20.19.4`, exact `https://lupi.live` origin   | Signed artifact of this source or on-device presentation                        |
| Source/assets gate          | `check:testflight` passed; strict gate passed tracked/clean scope checks and failed only because `submit.production.ios.ascAppId` is absent; icon/splash source properties passed                   | Fully green strict gate, App Store Connect ID, or rendered assets               |
| Final local ladder          | 105/105 tests, typecheck, zero-warning lint, `expo install --check`, Doctor 18/18, 20-route web export, clean unsigned iOS export, local visual contract, and EAS workflow schema validation passed | Native compile or device behavior                                               |
| Signed development artifact | Build `2b57a89e-e398-44a8-b799-871b7f8e3651` finished for exact clean `7c64bd70`, version/build `1.0.0 (1)`, signed for one registered iPhone                                                       | The integrated `1.0.1`/iOS `17.6` source, TestFlight, or physical AR acceptance |
| Development update          | Active on channel `development`, runtime `1.0.0`, group `0442ed9e-1ebc-4da0-a79c-8750e37641e8`, exact clean `7c64bd70`                                                                              | Device screenshot/acceptance; runtime `1.0.1` requires a new binary             |
...
| EAS archive                 | Fresh standalone archive passed for 92 files and 1,472,201 bytes, every byte matching current mobile/core source                                                                                    | Upload or EAS builder receipt for integrated source                             |
...
Expo SDK 54 includes the exact `react-native-webview` `13.15.0` line used here,
so this slice runs in Expo Go. See the [SDK 54 WebView reference](https://docs.expo.dev/versions/v54.0.0/sdk/webview/).

SDK 54 also marks `expo-document-picker` `~14.0.8` and `expo-file-system`
`~19.0.23` as included in Expo Go, matching the installed package lines. The
...
[SDK 54 DocumentPicker reference](https://docs.expo.dev/versions/v54.0.0/sdk/document-picker/)
and [SDK 54 FileSystem reference](https://docs.expo.dev/versions/v54.0.0/sdk/filesystem/).
...
- Node.js 20.19.x; the isolated candidate used `20.19.4`.
- pnpm 9; the isolated candidate used the repository-pinned `9.0.0`.
- The current Expo Go app from the iOS App Store.
...
Scan the QR code with the iPhone camera or Expo Go. On Windows, the `ios` script
cannot launch Apple's iOS Simulator; the physical-device QR path is the normal
workflow.
...
- Do not import `@atlas/ui` into mobile. It is the browser UI, owns DOM/browser
  assumptions, and resolves a different React patch line than Expo SDK 54.
...
Viro `2.56.0` is pinned because the current app remains on Expo SDK 54 / React
Native 0.81. Its Expo plugin unconditionally excludes `arm64` for iOS simulator
builds. Therefore the existing Apple-silicon `visual-ios` simulator workflow may
...
the [SDK 54 GLView reference](https://docs.expo.dev/versions/v54.0.0/sdk/gl-view/).

Install the spike as a deliberate follow-up change from the mobile app
directory, keeping the same Three/R3F lines as the web workspace while Expo 54
pins React 19.1:
...
The current app already has `newArchEnabled: true` and React Native 0.81, which
meet the library's documented baseline, but compatibility still has to be
...
Version/runtime `1.0.1` is selected in source, iOS deployment target `17.6` is
explicit, and remote iOS build number `1` is initialized. A signed development
binary exists only for earlier version `1.0.0 (1)` / revision `7c64bd70`; it does
...
### Integrated-candidate local/configuration receipts

| Check                            | Integrated-candidate result                                   | Evidence boundary                                                                                                                                                           |
| -------------------------------- | ------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| frozen filtered install          | Passed with Node 20.19.4 and pnpm 9.0.0                       | Exact lockfile and lifecycle scripts; not an EAS builder receipt                                                                                                            |
| local source ladder components   | Passed                                                        | Source gate, 105/105 tests, typecheck, zero-warning lint, Expo dependency check, Doctor 18/18, 20-route web export, and clean unsigned iOS export                           |
| `check:testflight`               | Passed                                                        | Source/config/assets only; App Store Connect ID remains an external release gate                                                                                            |
| `check:testflight:release`       | Scoped checks passed; one blocker                             | Required files were tracked and the mobile scope was clean; only `submit.production.ios.ascAppId` was missing                                                               |
| Focused unit tests               | 105/105 passed on 2026-08-10                                  | Includes strict AR scene/session/handoff/build-policy tests plus Gallery/catalog, history, menus, Library/Settings, runtime recovery, config, XYZ, bridge, and recent codec |
| TypeScript                       | Final pass                                                    | Static type receipt only                                                                                                                                                    |
| ESLint                           | Final pass, zero warnings                                     | Static lint receipt only                                                                                                                                                    |
| `expo install --check`           | Passed                                                        | Installed Expo dependency compatibility only                                                                                                                                |
| Expo Doctor                      | 18/18 checks passed                                           | Expo project diagnostics only                                                                                                                                               |
| `check:visual-workflow:local`    | Passed; 36 Maestro commands                                   | Gallery-backed Caffeine, exact public 30-tool manifest, camera-free Room intro, and native Gallery/Library/Settings screenshot contracts; cloud execution remains open      |
| EAS visual workflow schema       | Passed                                                        | Schema/configuration receipt only; `eas workflow:runs --json` returned `[]`, so no cloud execution or screenshots exist                                                     |
| `visual:web` composition matrix  | Passed; 12/12 captures and 63/63 semantic checks              | Three iPhone-sized Expo-web profiles, zero overflow/unexpected-runtime failures; explicitly not native iOS, device, AR, or TestFlight proof                                 |
| `export:web --clear`             | Passed; 20 routes                                             | Expo browser-fallback export only; native Viro remains behind a lazy platform boundary                                                                                      |
| browser fallback QA              | Passed at 320x693 and 390x844 after overlap fix               | Web header/tab geometry only; not native layout                                                                                                                             |
...
| `export:ios --clear`             | Passed; 1,271 modules, 3.43 MB HBC                            | Clean unsigned JS/assets export only; “clean” refers to regenerated export output                                                                                           |
| EAS production config resolution | Store/autoIncrement/Node 20.19.4/origin/project ID resolved   | Configuration resolution only; not a build                                                                                                                                  |
...
| Signed development build         | Finished: `2b57a89e-e398-44a8-b799-871b7f8e3651`              | Exact clean `7c64bd70`, version/build `1.0.0 (1)`, one registered iPhone; not the integrated `1.0.1`/iOS 17.6 source, TestFlight, or physical-AR acceptance                 |
...
| `check:eas-archive`              | Passed for 92 files and 1,472,201 bytes (~1.40 MiB)           | Fresh allowlisted standalone archive; every byte matches mobile/core source and required AR/config inputs are present                                                       |
...
contract, workflow schema, archive, and web/iOS exports were freshly rerun after
the integrated Room work. The signed development build and development update
remain receipts for earlier exact clean revision `7c64bd70`; neither proves the
new `1.0.1`/iOS `17.6` source. G1 remains open until the clean final integrated
SHA is recorded and the strict release gate receives the App Store Connect
numeric ID. Workflow execution, TestFlight, and physical Room AR remain open.
...
bundle identifier `live.lupi.app`, marketing/runtime version `1.0.1`, and iOS
deployment target `17.6`. The remote project is `@alexwelcing/lupi`.

`eas.json` pins EAS CLI `>=20.3.0`, `appVersionSource: remote`, Node `20.19.4`
for preview and production, preview internal distribution, production store
...
registered iPhone. It predates the `1.0.1`/iOS `17.6` source and is not a
...
current isolated-candidate archive contains 92 files totaling 1,472,201 bytes
(about 1.40 MiB), excluding tests, docs, generated exports, dependencies, and
...
signed artifact, or EAS build.

### First-beta update and reviewer boundary

The app includes `expo-updates`, an app-version runtime policy, the linked EAS
...
device screenshot or acceptance receipt exists. The new `1.0.1` runtime needs a
new compatible binary before it can receive `1.0.1` updates. Viro, permissions,
...
- A current SDK/toolchain check. As of this compatibility snapshot, Apple
  requires iOS uploads to use the iOS 26 SDK, and Expo states that SDK 54's
  default EAS image uses Xcode 26. Recheck both requirements for the actual
  release date.
...
| ----------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------- |
| Isolated candidate            | Named/reviewed clean branch; Node 20.19.4/pnpm 9.0.0 frozen filtered install; full local ladder; Doctor; resolved config; byte-identical reviewed archive; scoped release checks | Final post-amendment SHA ledger, EAS build, Apple, or device behavior  |
...
| Expo/EAS configuration        | Authenticated owner; linked project ID; resolved store/autoIncrement/Node/origin profile; remote build number `1`                                                                | A queued or successful EAS build                                       |
